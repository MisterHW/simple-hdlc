...
# table generator function (provided for clarity. See also: RFC1662 appendix C.2)
def init_crc16_8408_tab():
    for i in range(256):
        crc = 0
        c = i
        poly = 0x8408
...
CRCCCIT_START    = 0xFFFF
CRCCCIT_POLY     = 0x1021 

CRC16_CCITT_FALSE_TAB = [
...
# table generator function (provided for clarity)
def init_crc16_ccitt_false_tab():
    for i in range(256):
        crc = i << 8

        for _ in range(8):
            if crc & 0x8000:
                crc = ((crc << 1) ^ CRCCCIT_POLY) & 0xFFFF
            else:
                crc = (crc << 1) & 0xFFFF
